...
public sealed class PersistedMessageScheduler : IMessageScheduler, IAsyncDisposable
{
        ...
        where TMessage : class, IMessage
    {
        ...
        var stored = new ScheduledMessage(
            ...
            body,
            new ScheduledMessageTimestamps(scheduledTime),
            headers,
    ...
    private async Task ProcessDueMessageAsync(IScheduledMessage message, CancellationToken cancellationToken)
    {
        ...
        (object? payload, string? deserializeError) = TryDeserializePayload(message, messageType!);
        if (await TryHandleInvalidMessageAsync(message, deserializeError, cancellationToken).ConfigureAwait(false))
            return;

        if (!TryResolveDestinationAddress(message, out Uri? destinationAddress, out string? invalidReason) && await TryHandleInvalidMessageAsync(message, invalidReason!, cancellationToken).ConfigureAwait(false))
            return;
    ...
    private (object? payload, string? error) TryDeserializePayload(IScheduledMessage message, Type messageType)
    {
        try
        {
            object payload = _serializer.Deserialize(message.Body.Span, messageType);
            return (payload, null);
        }
        catch (Exception ex)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var deadLetterMessage = new TransportMessage(
            ...
            new MessageIdentity(Ulid.NewUlid()),
            message.MessageType,
