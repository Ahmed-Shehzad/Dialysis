...
public sealed class EntityFrameworkInboxStore : IInboxStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        return string.IsNullOrWhiteSpace(consumerId)
            ...
            : (IInboxState?)await _context.Set<InboxStateRecord>()
            .AsNoTracking()
            .FirstOrDefaultAsync(
    ...
    public async Task<bool> TryAddAsync(IInboxState state, CancellationToken cancellationToken = default)
    {
        ...
        InboxStateRecord? existing = await _context.Set<InboxStateRecord>()
        ...
        var entity = InboxStateRecord.FromState(state);

        try
        {
            _ = await _context.Set<InboxStateRecord>()
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        InboxStateRecord? state = await _context.Set<InboxStateRecord>()
            .FirstOrDefaultAsync(
