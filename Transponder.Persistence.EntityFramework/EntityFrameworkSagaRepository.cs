    ...
    where TState : class, ISagaState
{
    ...
    {
        SagaStateRecord? entity = await _context.Set<SagaStateRecord>()
            .AsNoTracking()
            .FirstOrDefaultAsync(
    ...
    public async Task<bool> SaveAsync(TState state, CancellationToken cancellationToken = default)
    {
        ...
        SagaStateRecord? existing = await _context.Set<SagaStateRecord>()
            .FirstOrDefaultAsync(
        ...
        {
            state.Version = 1;
            var entity = SagaStateRecord.FromState(state);
            _ = await _context.Set<SagaStateRecord>()
        ...
        state.Version++;
        var updated = SagaStateRecord.FromState(state);
    ...
    public async Task DeleteAsync(Ulid correlationId, CancellationToken cancellationToken = default)
    {
        ...
        _ = _context.Set<SagaStateRecord>().Remove(entity);
    }
}
