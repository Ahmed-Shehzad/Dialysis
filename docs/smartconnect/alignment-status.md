...
| ~~D2~~ | ~~Inbound File Reader emits one message per record + `WithBatch(…)`~~ | **Shipped.** `FileReaderParameters.SplitMode` covers `None / Hl7v2 / Line / Regex / DelimitedTextRecords`; per-record dispatch loop in `FileReaderSourceConnector.ProcessFileAsync` tags each record with `BatchMetadataKeys.{BatchId,Sequence,Total,Source}` via the `WithBatch(…)` extension. Indexed `BatchId` column on `MessageLedgerEntryEntity` + `?batchId=` query on `GET /admin/messages` + Messages-tab filter + per-row "batch n/total" badge close the consumer side. The `DelimitedTextRecords` mode shares the L2 streaming parser via `Core.DataTypes.DelimitedTextStreaming`. |
