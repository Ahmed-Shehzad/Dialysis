...
- Map MSH-3/OBR-3 to Device (DeviceMapper)
- Map OBX to Observation with MDC code, UCUM unit (ObservationMapper, MdcToFhirCodeCatalog, UcumMapper)
- Support focus on Device per QI-Core NonPatient pattern
- Map OBX-17 provenance to note or Provenance resource
- Expose Device via GET /api/devices/{id}/fhir
- Expose Observation in treatment-session FHIR Bundle
...
- Map Prescription aggregate to ServiceRequest (PrescriptionMapper)
- Include therapy modality, UF target, flow rates
- Expose via GET /api/prescriptions/{mrn}/fhir
...
- Map TreatmentSession to Procedure (ProcedureMapper)
- Map status (Active → in-progress, Completed → completed)
- Include performedPeriod, Device reference
- Expose in GET /api/treatment-sessions/{id}/fhir Bundle
...
- Map ORU^R40 alarms to DetectedIssue (AlarmMapper)
- Map severity from OBX-8 priority
- Map interpretation type (SP/ST/SA)
- Expose via GET /api/alarms/fhir
...
- Map PID to Patient (PatientMapper)
- Expose via GET /api/patients/{mrn}/fhir

#### 3.5.2 Provenance

- Map OBX-17 (RSET/MSET/ASET) to Provenance (ProvenanceMapper)
- Link to target Observation

#### 3.5.3 AuditEvent

- C5 audit for HL7 ingest, prescription download (AuditEventMapper)
- Expose via GET /api/audit-events
...
- GET /api/fhir/$export – NDJSON stream
- Supported types: Patient, Device, ServiceRequest, Procedure, Observation, DetectedIssue, AuditEvent
...
- REST-hook channel
- Criteria: resource type (Observation, Procedure, DetectedIssue)
- Notify on TreatmentSessionStarted, ObservationRecorded, AlarmRaised

#### 3.6.4 CDS

- GET /api/cds/prescription-compliance – Prescription vs treatment deviation
- Returns DetectedIssue when outside tolerance
...
| OBX-6 units | [http://unitsofmeasure.org](http://unitsofmeasure.org) | mmHg → mm[Hg], ml/min → mL/min              |
| LOINC       | [http://loinc.org](http://loinc.org)                   | 8480-6 (Systolic BP), 8462-4 (Diastolic BP) |
| SNOMED CT   | [http://snomed.info/sct](http://snomed.info/sct)       | 1088001 (Hemodialysis)                      |
