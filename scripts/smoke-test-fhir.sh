...
code=$(curl -s -o /dev/null -w "%{http_code}" -H "X-Tenant-Id: $TENANT" "$BASE_URL/api/fhir/\$export?_type=Patient,Device&_limit=5")
[[ "$code" == "200" ]] && echo "OK ($code)" || { echo "FAIL ($code)"; exit 1; }

# FHIR Subscription create
echo -n "POST /api/fhir/Subscription ... "
resp=$(curl -s -X POST -H "X-Tenant-Id: $TENANT" -H "Content-Type: application/fhir+json" -H "Accept: application/fhir+json" \
...
code=$(curl -s -o /dev/null -w "%{http_code}" -H "X-Tenant-Id: $TENANT" "$BASE_URL/api/reports/sessions-summary")
[[ "$code" == "200" ]] && echo "OK ($code)" || { echo "FAIL ($code)"; exit 1; }

if $DO_HL7; then
  # Patient QBP^Q22 (PDQ)
  echo -n "POST /api/hl7/qbp-q22 ... "
  qbp_resp=$(curl -s -X POST -H "X-Tenant-Id: $TENANT" -H "Content-Type: application/json" \
  ...
  d01_resp=$(curl -s -X POST -H "X-Tenant-Id: $TENANT" -H "Content-Type: application/json" \
