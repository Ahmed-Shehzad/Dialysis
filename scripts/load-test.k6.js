 ...
 *   K6_TIMEOUT (default 30s), K6_FAIL_RATE (default 0.05; use 0.10+ for heavier load)
 */
...
const REQUEST_TIMEOUT = __ENV.K6_TIMEOUT || '30s';
const FAIL_RATE_THRESHOLD = parseFloat(__ENV.K6_FAIL_RATE || '0.05');
...
const reqOpts = (h) => ({ headers: h, timeout: REQUEST_TIMEOUT });

export default function () {
  const commonHeaders = { ...headers(), 'Accept': 'application/fhir+json' };

  // Health
  let res = http.get(`${BASE_URL}/health`, reqOpts(headers()));
  check(res, { 'health status 200': (r) => r.status === 200 });
  sleep(0.1);

  // FHIR export (aggregates Patient, Device; may be slower under load)
  res = http.get(`${BASE_URL}/api/fhir/$export?_type=Patient,Device&_limit=5`, reqOpts(commonHeaders));
  ...
  // CDS (404 valid when session does not exist)
  res = http.get(`${BASE_URL}/api/cds/prescription-compliance?sessionId=SESS001`, reqOpts(headers()));
  check(res, { 'cds status 200 or 404': (r) => r.status === 200 || r.status === 404 });
  sleep(0.1);

  // Reports
  res = http.get(`${BASE_URL}/api/reports/sessions-summary`, reqOpts(headers()));
