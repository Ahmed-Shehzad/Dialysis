...
public class IntercessorBuilder
{
    ...
    internal void Build()
    {
        ...
        {
            foreach (var assembly in distinctAssemblies) x.RegisterFromAssembly(assembly);
        });
    ...
    {
        foreach (var behaviorType in behaviorTypes) RegisterPipelineBehavior(behaviorType);
    }

    private void RegisterPipelineBehavior(Type behaviorType)
    {
        var isOpenGeneric = behaviorType.ContainsGenericParameters;

        foreach (var interfaceType in behaviorType.GetInterfaces())
        {
            if (!IsPipelineBehaviorInterface(interfaceType)) continue;

            var serviceType = isOpenGeneric
