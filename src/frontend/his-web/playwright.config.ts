...
/**
 * his-web UI end-to-end tests. They run against the Vite dev server (served under base `/his/`);
 ...
 * `e2e-artifacts/his-web/` (gitignored). Playwright clears `outputDir` at the start of every run, so
 * each run overwrites the previous run's artifacts — bounded disk use, always the latest walkthrough.
 */
export default defineConfig({
  testDir: "./e2e",
  outputDir: "../../../e2e-artifacts/his-web/test-results",
  fullyParallel: true,
  forbidOnly: !!process.env.CI,
  retries: process.env.CI ? 1 : 0,
  reporter: process.env.CI
    ? [["github"], ["html", { open: "never", outputFolder: "../../../e2e-artifacts/his-web/report" }]]
    : [["list"], ["html", { open: "never", outputFolder: "../../../e2e-artifacts/his-web/report" }]],
  ...
  use: {
    baseURL: "http://localhost:5331/his/",
