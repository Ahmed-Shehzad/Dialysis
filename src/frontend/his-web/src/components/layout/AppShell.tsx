...
// Single-context (HIS) navigation. Paths are absolute within the app's `/his` router basename.
const HIS_NAV: ReadonlyArray<{ to: string; label: string }> = [
...
// Front Desk (HIS) chrome. The top nav lists this context's views; cross-context navigation
      ...
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          <div className="flex items-center gap-6">
            <h1 className="text-lg font-semibold tracking-tight text-clinic-50">Dialysis · Front Desk</h1>
            <nav className="flex flex-wrap gap-1" aria-label="Front Desk">
              {HIS_NAV.map((item) => (
