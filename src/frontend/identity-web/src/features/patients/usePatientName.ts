...
export const usePatientDemographics = (patientId: string | null | undefined) => {
  const query = useQuery({
    ...
    // The loader coalesces every concurrent id into one batched POST — see patientLoader.ts. One header
    // or 50 list rows both cost a single request per tick.
    queryFn: () => loadPatientLabel(patientId as string),
    enabled: Boolean(patientId),
    staleTime: 5 * 60_000,
    gcTime: 30 * 60_000,
    // A failed/missing label degrades to the id placeholder in the UI; don't retry-storm.
    retry: false,
  });
