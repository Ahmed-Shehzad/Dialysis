...
export const fetchCurrentUser = async (): Promise<AuthenticatedUser> => {
  const response = await apiClient.get<{
    ...
    claims?: Record<string, unknown>;
    accessToken?: string;
  }>("/admin/identity/user", { timeout: AUTH_PROBE_TIMEOUT_MS });
...
export const buildLoginUrl = (returnPath = "/", provider?: string): string => {
  const url = "/admin/identity/login?returnUrl=" + encodeURIComponent(buildReturnTarget(returnPath));
  return provider ? url + "&provider=" + encodeURIComponent(provider) : url;
};

export const buildLogoutUrl = (returnPath = "/"): string =>
  "/admin/identity/logout?returnUrl=" + encodeURIComponent(buildReturnTarget(returnPath));
