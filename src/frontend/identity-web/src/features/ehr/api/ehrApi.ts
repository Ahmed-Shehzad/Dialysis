...
export const searchEhrPatientsPage = async (
  ...
  const response = await apiClient.get<PatientSearchPage>("/admin/api/_x/ehr/api/v1.0/patients", {
...
export const fetchPatientChart = async (patientId: string): Promise<PatientChartView> => {
  const response = await apiClient.get<PatientChartView>(
    `/admin/api/_x/ehr/api/v1.0/patients/${patientId}/chart`,
  );
...
export const fetchPatientNotes = async (
  ...
  const response = await apiClient.get<ClinicalNoteListItem[]>(
    `/admin/api/_x/ehr/api/v1.0/patients/${patientId}/notes`,
    { params: { take } },
  );
  return response.data ?? [];
};
...
export const fetchPatientLabResults = async (
  ...
  const response = await apiClient.get<LabResultListItem[]>(
    `/admin/api/_x/ehr/api/v1.0/patients/${patientId}/lab-results`,
...
export const fetchEhrPatient = async (patientId: string): Promise<EhrPatientDetail | null> => {
  try {
    const response = await apiClient.get<EhrPatientDetail>(
      `/admin/api/_x/ehr/api/v1.0/patients/${patientId}`,
    );
    return response.data;
  } catch (error) {
 ...
 * through the admin BFF's _x/ehr aggregation. Ids go in the request BODY, never the query string, to keep
 * patient identifiers out of gateway / proxy access logs. This is the N+1 killer behind the resolver.
 */
export const fetchPatientsByIds = async (ids: string[]): Promise<PatientLabel[]> => {
  ...
  const pages = await Promise.all(
    chunks.map((chunk) =>
      apiClient
        .post<PatientLabel[]>(`/admin/api/_x/ehr/api/v1.0/patients/by-ids`, { ids: chunk })
        .then((r) => r.data ?? []),
    ),
  );
  return pages.flat();
...
export const registerPatient = async (body: RegisterPatientRequest): Promise<string> => {
  const response = await apiClient.post<{ id: string }>(
    "/admin/api/_x/ehr/api/v1.0/clinical/patients",
    body,
  );
...
export const startEncounter = async (body: StartEncounterRequest): Promise<string> => {
  const response = await apiClient.post<{ id: string }>(
    "/admin/api/_x/ehr/api/v1.0/clinical/encounters",
    body,
  );
  return response.data.id;
};
...
export const signClinicalNote = async (body: SignNoteRequest): Promise<void> => {
  await apiClient.post(`/admin/api/_x/ehr/api/v1.0/clinical/notes/${body.noteId}/sign`, {
...
export const orderLabTest = async (body: OrderLabTestRequest): Promise<string> => {
  const response = await apiClient.post<{ id: string }>(
    "/admin/api/_x/ehr/api/v1.0/clinical/lab-orders",
    body,
  );
  return response.data.id;
};
...
export const draftClinicalNote = async (body: DraftClinicalNoteRequest): Promise<string> => {
  const response = await apiClient.post<{ id: string }>(
    "/admin/api/_x/ehr/api/v1.0/clinical/notes/draft",
    body,
  );
  return response.data.id;
};
