...
test.describe("identity-web — authenticated landing", () => {
  test("loads the admin landing context for an authenticated user", async ({ page }) => {
    await stubRealtime(page);
    await stubApiCatchAll(page);
    await mockAuth(page, { permissions: ["identity.user.read"] });
