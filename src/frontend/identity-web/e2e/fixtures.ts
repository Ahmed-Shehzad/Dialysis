...
export async function mockAuth(page: Page, user: MockUser = {}): Promise<void> {
  await page.route("**/admin/identity/user", (route) =>
    route.fulfill({
      json: {
...
export async function stubRealtime(page: Page): Promise<void> {
  await page.route("**/admin/hubs/**", (route) => route.abort());
  await page.route("**/admin/events/**", (route) => route.abort());
}

/**
 * Aborts any `/admin/api` call a panel makes that a spec did not explicitly mock, so the SPA renders
