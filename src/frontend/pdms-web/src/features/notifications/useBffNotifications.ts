...
// BFF-owned notifications hub for this context (gateway-routed to the PDMS BFF, cookie-authenticated
// on the same origin). The BFF consumes integration events off RabbitMQ and pushes them here.
const HUB_URL = "/pdms/events";
 ...
 * and surfaces each push as a toast (chairside alarms render as error toasts). Best-effort: if the
 * hub is unavailable it silently no-ops. Re-subscribes when the selected patient changes.
 */
