import { fetchPatientsByIds, type PatientLabel } from "@/features/ehr/api/ehrApi";
 ...
 * request instead of N. It is wired as the TanStack Query `queryFn` for `["ehr","patient-name", id]`, so
 * per-id caching (dedup of the same id) and cross-id HTTP batching (different ids) combine — and every
 * existing `usePatientDemographics` call site becomes N+1-free with no change.
 */
const createPatientLoader = () => {
  ...
  return (id: string): Promise<PatientLabel | null> =>
    new Promise<PatientLabel | null>((resolve) => {
      ...
      if (!scheduled) {
        scheduled = true;
        // Defer to a macrotask so the whole synchronous render burst (e.g. 50 list rows mounting at once)
        // is coalesced into one batch before it flushes.
        setTimeout(flush, 0);
      }
    });
};
