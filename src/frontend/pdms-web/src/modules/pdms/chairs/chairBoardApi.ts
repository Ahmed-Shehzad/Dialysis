...
/**
 * Wire shape of PDMS's `GET /pdms/api/v1.0/chairs`. Each entry is one chair currently
...
const fetchChairAssignments = async (): Promise<readonly ChairAssignment[]> => {
  const response = await apiClient.get<ChairAssignment[]>("/pdms/api/v1.0/chairs");
