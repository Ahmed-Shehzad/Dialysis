...
const fetchActiveAlarms = async (): Promise<readonly AlarmListItem[]> => {
  const response = await apiClient.get<AlarmListItem[]>("/pdms/api/v1.0/alarms");
  return response.data ?? [];
};

/**
 * Polls `GET /pdms/api/v1.0/alarms` every 5 seconds for active alarms. Polling is the
...
const acknowledgeAlarm = async (alarmId: string): Promise<void> => {
  await apiClient.post(`/pdms/api/v1.0/alarms/${alarmId}/acknowledge`);
};
