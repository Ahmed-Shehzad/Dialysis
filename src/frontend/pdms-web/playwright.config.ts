...
/**
 * pdms-web UI end-to-end tests. They run against the Vite dev server (served under base `/pdms/`);
 ...
 * `e2e-artifacts/pdms-web/` (gitignored). Playwright clears `outputDir` at the start of every run, so
 * each run overwrites the previous run's artifacts — bounded disk use, always the latest walkthrough.
 */
export default defineConfig({
  testDir: "./e2e",
  outputDir: "../../../e2e-artifacts/pdms-web/test-results",
  fullyParallel: true,
  ...
  use: {
    baseURL: "http://localhost:5333/pdms/",
