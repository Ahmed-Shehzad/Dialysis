...
/** Minimal shape of the BFF `/smartconnect/identity/user` probe response stubbed in tests. */
...
export async function mockAuth(page: Page, user: MockUser = {}): Promise<void> {
  await page.route("**/smartconnect/identity/user", (route) =>
    route.fulfill({
      json: {
...
export async function stubRealtime(page: Page): Promise<void> {
  await page.route("**/smartconnect/hubs/**", (route) => route.abort());
  await page.route("**/smartconnect/events/**", (route) => route.abort());
}

/**
 * Aborts any `/smartconnect/api` call a panel makes that a spec did not explicitly mock, so the SPA renders
...
export async function stubApiCatchAll(page: Page): Promise<void> {
  await page.route("**/smartconnect/api/**", (route) => route.abort());
}
