...
test.describe("smartconnect-web — authenticated landing", () => {
  test("loads the smartconnect landing context for an authenticated user", async ({ page }) => {
    await stubRealtime(page);
    await stubApiCatchAll(page);
    await mockAuth(page, { permissions: ["smartconnect.feeds.view"] });

    await page.goto("/smartconnect/");
