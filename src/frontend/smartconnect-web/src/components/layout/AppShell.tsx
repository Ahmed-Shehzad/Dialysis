...
// Single-context (SmartConnect) navigation. Paths are absolute within the app's `/smartconnect` router basename.
const SC_NAV: ReadonlyArray<{ to: string; label: string }> = [
...
// Feeds (SmartConnect) chrome. The top nav lists this context's views; cross-context navigation
      ...
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          <div className="flex items-center gap-6">
            <h1 className="text-lg font-semibold tracking-tight text-clinic-50">Dialysis · Feeds</h1>
            <nav className="flex flex-wrap gap-1" aria-label="Feeds">
              {SC_NAV.map((item) => (
