 ...
 * `e2e-artifacts/ehr-web/` (gitignored). Playwright clears `outputDir` at the start of every run, so
 * each run overwrites the previous run's artifacts — bounded disk use, always the latest walkthrough.
 */
export default defineConfig({
  testDir: "./e2e",
  outputDir: "../../../e2e-artifacts/ehr-web/test-results",
  fullyParallel: true,
  forbidOnly: !!process.env.CI,
  retries: process.env.CI ? 1 : 0,
  reporter: process.env.CI
    ? [["github"], ["html", { open: "never", outputFolder: "../../../e2e-artifacts/ehr-web/report" }]]
    : [["list"], ["html", { open: "never", outputFolder: "../../../e2e-artifacts/ehr-web/report" }]],
