...
/** Minimal shape of the BFF `/ehr/identity/user` probe response stubbed in tests. */
...
export async function mockAuth(page: Page, user: MockUser = {}): Promise<void> {
  await page.route("**/ehr/identity/user", (route) =>
    route.fulfill({
      json: {
...
export async function stubRealtime(page: Page): Promise<void> {
  await page.route("**/ehr/hubs/**", (route) => route.abort());
  await page.route("**/ehr/events/**", (route) => route.abort());
}

/**
 * Aborts any `/ehr/api` call a panel makes that a spec did not explicitly mock, so the SPA renders
...
export async function stubApiCatchAll(page: Page): Promise<void> {
  await page.route("**/ehr/api/**", (route) => route.abort());
}
