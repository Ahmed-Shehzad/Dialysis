...
export const searchEhrPatientsPage = async (
  ...
  const response = await apiClient.get<PatientSearchPage>("/ehr/api/v1.0/patients", {
...
export const fetchPatientChart = async (patientId: string): Promise<PatientChartView> => {
  const response = await apiClient.get<PatientChartView>(
    `/ehr/api/v1.0/patients/${patientId}/chart`,
  );
...
export const fetchPatientNotes = async (
  ...
  const response = await apiClient.get<ClinicalNoteListItem[]>(
    `/ehr/api/v1.0/patients/${patientId}/notes`,
    { params: { take } },
  );
  return response.data ?? [];
};
...
export const fetchPatientLabResults = async (
  ...
  const response = await apiClient.get<LabResultListItem[]>(
    `/ehr/api/v1.0/patients/${patientId}/lab-results`,
 ...
 * ehr-web owns demographics, so this hits the EHR API directly (no aggregation). Ids go in the request
 * BODY, never the query string, to keep patient identifiers out of gateway / proxy access logs. This is
 * the N+1 killer behind the batched patient-label resolver.
 */
export const fetchPatientsByIds = async (ids: string[]): Promise<PatientLabel[]> => {
  ...
  const pages = await Promise.all(
    chunks.map((chunk) =>
      apiClient
        .post<PatientLabel[]>(`/ehr/api/v1.0/patients/by-ids`, { ids: chunk })
        .then((r) => r.data ?? []),
    ),
  );
  return pages.flat();
...
export const signClinicalNote = async (body: SignNoteRequest): Promise<void> => {
  await apiClient.post(`/ehr/api/v1.0/clinical/notes/${body.noteId}/sign`, {
