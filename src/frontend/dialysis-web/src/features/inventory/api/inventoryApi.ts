...
export const fetchInventory = async (lowStockOnly = false): Promise<InventoryItem[]> => {
  const response = await apiClient.get<InventoryItem[]>("/api/pdms/api/v1.0/inventory", {
    params: { lowStockOnly },
  });
...
export const receiveStock = async (
  ...
  const response = await apiClient.post<InventoryItem>(
    `/api/pdms/api/v1.0/inventory/${id}/receive`,
    { units, reason },
  );
  return response.data;
};
...
export const adjustStock = async (
  ...
  const response = await apiClient.post<InventoryItem>(
    `/api/pdms/api/v1.0/inventory/${id}/adjust`,
