...
/// </summary>
public sealed class HieTefcaSandboxSeeder(
    IServiceProvider services,
    ILogger<HieTefcaSandboxSeeder> logger) : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning("HIE TEFCA sandbox seeder: database not reachable, skipping.");
            return;
        }

        if (await db.QhinPartners.AnyAsync(p => p.Id == _SandboxPartnerId, cancellationToken).ConfigureAwait(false))
        {
            logger.LogInformation("HIE TEFCA sandbox seeder: sandbox partner already present, skipping.");
            return;
        }
