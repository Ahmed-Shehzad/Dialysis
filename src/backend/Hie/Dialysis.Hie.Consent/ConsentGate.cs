...
/// </summary>
public sealed class ConsentGate(IConsentRepository repository, TimeProvider timeProvider) : IConsentGate
{
    public async Task<bool> CheckOutboundAsync(Guid patientId, string partnerId, string scope, CancellationToken cancellationToken = default)
    {
        ...
        var bidirectional = await repository
