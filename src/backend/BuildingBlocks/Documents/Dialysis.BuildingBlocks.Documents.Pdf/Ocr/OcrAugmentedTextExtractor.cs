...
/// </summary>
public sealed class OcrAugmentedTextExtractor : IPdfTextExtractor
{
    ...
    /// gracefully to a pure-extraction pass.
    /// </summary>
    public OcrAugmentedTextExtractor(IPdfTextExtractor textExtractor,
    ...
    {
        var native = await _textExtractor.ExtractAsync(pdfDocument, cancellationToken).ConfigureAwait(false);
        if (_rasterizer is null || _ocrEngine is null)
            return native;
        ...
        await foreach (var raster in _rasterizer.RasterizeAsync(pdfDocument, RasterizationOptions.OcrDefault, cancellationToken).ConfigureAwait(false))
        {
            if (!emptyPages.Contains(raster.PageNumber))
                continue;
            var ocrResult = await _ocrEngine.RecognizeAsync(raster.Bytes, OcrOptions.Multilingual, cancellationToken)
