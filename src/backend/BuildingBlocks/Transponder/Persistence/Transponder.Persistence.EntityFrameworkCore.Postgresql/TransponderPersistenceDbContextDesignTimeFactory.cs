...
/// and resolves the connection string the same way as runtime. Use <c>dotnet ef --startup-project &lt;your host&gt;</c> so the working directory and appsettings match the client app.
/// </summary>
public sealed class TransponderPersistenceDbContextDesignTimeFactory : IDesignTimeDbContextFactory<TransponderPersistenceDbContext>
{
    public TransponderPersistenceDbContext CreateDbContext(string[] args)
    {
        ...
        var options = new DbContextOptionsBuilder<TransponderPersistenceDbContext>()
            .UseNpgsql(
                connectionString,
                npgsql => npgsql.MigrationsHistoryTable("__EFMigrationsHistory", persistenceOptions.Schema))
            .Options;
