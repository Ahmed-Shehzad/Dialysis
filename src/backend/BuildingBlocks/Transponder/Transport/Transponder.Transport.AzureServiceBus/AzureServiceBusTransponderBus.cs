...
public sealed class AzureServiceBusTransponderBus(
    ITransponderTransport transport,
    IMessageSerializer serializer,
    ILogger<AzureServiceBusTransponderBus> logger) : ITransponderBus
{
        ...
        where TMessage : class
    {
        ...
        logger.LogDebug("Published {MessageType} to Azure Service Bus with correlation {CorrelationId}", typeof(TMessage).Name, correlation);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await transport.PublishAsync(envelope, cancellationToken).ConfigureAwait(false);
        logger.LogDebug("Published {RoutingKey} to Azure Service Bus with correlation {CorrelationId}", routingKey, correlation);
    }
