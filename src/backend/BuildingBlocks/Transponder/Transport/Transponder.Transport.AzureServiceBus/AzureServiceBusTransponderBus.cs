...
/// <summary>Publishes JSON payloads to an Azure Service Bus topic using <see cref="TransponderTransportHeaderNames"/> for CLR routing.</summary>
public sealed class AzureServiceBusTransponderBus : ITransponderBus
{
    private readonly ITransponderTransport _transport;
    private readonly IMessageSerializer _serializer;
    private readonly ILogger<AzureServiceBusTransponderBus> _logger;
    /// <summary>Publishes JSON payloads to an Azure Service Bus topic using <see cref="TransponderTransportHeaderNames"/> for CLR routing.</summary>
    public AzureServiceBusTransponderBus(ITransponderTransport transport,
        IMessageSerializer serializer,
        ILogger<AzureServiceBusTransponderBus> logger)
    {
        ...
        where TMessage : class
    {
        ...
        _logger.LogDebug("Published {MessageType} to Azure Service Bus with correlation {CorrelationId}", typeof(TMessage).Name, correlation);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _transport.PublishAsync(envelope, cancellationToken).ConfigureAwait(false);
        _logger.LogDebug("Published {RoutingKey} to Azure Service Bus with correlation {CorrelationId}", routingKey, correlation);
    }
