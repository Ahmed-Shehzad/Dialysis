...
/// <summary>Publishes JSON payloads to a Transponder gRPC ingress relay.</summary>
public sealed class GrpcTransponderBus : ITransponderBus
{
    private readonly ITransponderTransport _transport;
    private readonly IMessageSerializer _serializer;
    private readonly ILogger<GrpcTransponderBus> _logger;
    /// <summary>Publishes JSON payloads to a Transponder gRPC ingress relay.</summary>
    public GrpcTransponderBus(ITransponderTransport transport,
        IMessageSerializer serializer,
        ILogger<GrpcTransponderBus> logger)
    {
        ...
        where TMessage : class
    {
        ...
        _logger.LogDebug("Published {MessageType} via gRPC with correlation {CorrelationId}", typeof(TMessage).Name, correlation);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _transport.PublishAsync(envelope, cancellationToken).ConfigureAwait(false);
        _logger.LogDebug("Published {RoutingKey} via gRPC with correlation {CorrelationId}", routingKey, correlation);
    }
