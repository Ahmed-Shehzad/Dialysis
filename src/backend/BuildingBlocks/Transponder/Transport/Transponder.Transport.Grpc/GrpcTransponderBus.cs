...
public sealed class GrpcTransponderBus(
    ITransponderTransport transport,
    IMessageSerializer serializer,
    ILogger<GrpcTransponderBus> logger) : ITransponderBus
{
        ...
        where TMessage : class
    {
        ...
        logger.LogDebug("Published {MessageType} via gRPC with correlation {CorrelationId}", typeof(TMessage).Name, correlation);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await transport.PublishAsync(envelope, cancellationToken).ConfigureAwait(false);
        logger.LogDebug("Published {RoutingKey} via gRPC with correlation {CorrelationId}", routingKey, correlation);
    }
