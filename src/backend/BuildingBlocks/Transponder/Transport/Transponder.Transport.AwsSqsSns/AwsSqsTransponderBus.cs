...
public sealed class AwsSqsTransponderBus(
    ITransponderTransport transport,
    IMessageSerializer serializer,
    ILogger<AwsSqsTransponderBus> logger) : ITransponderBus
{
        ...
        where TMessage : class
    {
        ...
        logger.LogDebug("Published {MessageType} to SQS with correlation {CorrelationId}", typeof(TMessage).Name, correlation);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await transport.PublishAsync(envelope, cancellationToken).ConfigureAwait(false);
        logger.LogDebug("Published {RoutingKey} to SQS with correlation {CorrelationId}", routingKey, correlation);
    }
