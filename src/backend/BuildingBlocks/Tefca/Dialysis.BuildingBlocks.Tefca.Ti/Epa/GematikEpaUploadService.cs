...
/// </summary>
public sealed class GematikEpaUploadService(
    ITelematikInfrastrukturClient ti,
    IPatientConsentGateway consent,
    IGematikAccessTokenSource tokens,
    IHttpClientFactory httpClientFactory,
    ILogger<GematikEpaUploadService> logger) : IEpaUploadService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var decision = await consent.AuthoriseAsync(
            request.PatientId,
        ...
        try
        {
            var endpoint = GematikEndpointCatalog.For(ti.Environment);
            var accessToken = await tokens.GetAccessTokenAsync(cancellationToken).ConfigureAwait(false);

            using var client = httpClientFactory.CreateClient("tefca-ti-epa-upload");
        ...
        {
            logger.LogError(ex, "ePA upload threw for patient {PatientId}.", request.PatientId);
