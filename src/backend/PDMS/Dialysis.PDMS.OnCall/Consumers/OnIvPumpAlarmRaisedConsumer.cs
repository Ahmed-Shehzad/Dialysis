...
/// </summary>
public sealed class OnIvPumpAlarmRaisedConsumer(
    IOnCallRotationLookup rotations,
    IEscalationPolicyLookup policies,
    IClinicianNotificationDispatcher dispatcher,
    IAlarmDispatchRepository dispatches,
    IUnitOfWork unitOfWork,
    TimeProvider clock) : IConsumer<IvPumpAlarmRaisedIntegrationEvent>
{
    public async Task HandleAsync(ConsumeContext<IvPumpAlarmRaisedIntegrationEvent> context)
    {
        var alarm = context.Message;
        var rotation = await rotations.FindActiveAsync(alarm.ChairId, alarm.RaisedAtUtc, context.CancellationToken)
        ...
        var policy = await policies.FindActiveAsync(context.CancellationToken).ConfigureAwait(false)
        ...
        var outcomes = await dispatcher.DispatchAsync(requests, context.CancellationToken).ConfigureAwait(false);
