...
/// </summary>
public sealed class ConfigurableCptFeeSchedule : ICptFeeSchedule
{
    ...
    /// charge consumer never blocks waiting on a fee schedule.
    /// </summary>
    public ConfigurableCptFeeSchedule(IConfiguration configuration) => _configuration = configuration;

    public Task<Money> LookupAsync(string cptCode, CancellationToken cancellationToken)
    {
        var money = _cache.GetOrAdd(cptCode, code =>
        {
            var section = _configuration.GetSection($"EHR:Billing:CptFeeSchedule:{code}");
