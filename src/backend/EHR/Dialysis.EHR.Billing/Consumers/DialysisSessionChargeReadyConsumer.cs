...
/// </summary>
public sealed class DialysisSessionChargeReadyConsumer(
    ...
    : IConsumer<DialysisSessionChargeReadyIntegrationEvent>
{
    public async Task HandleAsync(ConsumeContext<DialysisSessionChargeReadyIntegrationEvent> context)
    {
        var message = context.Message;
        var ct = context.CancellationToken;
        var existing = await idempotency.FindChargeIdAsync(message.SessionId, message.CptCode, ct)
        ...
        var billed = await feeSchedule.LookupAsync(message.CptCode, ct).ConfigureAwait(false);
        ...
        await idempotency.RegisterAsync(message.SessionId, message.CptCode, chargeId, ct).ConfigureAwait(false);
