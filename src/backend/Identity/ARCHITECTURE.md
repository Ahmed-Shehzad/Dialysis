...
```mermaid
flowchart LR
    Admin([Admin console SPA /admin]):::ui --> ABFF[Admin BFF :5306]
    B([Browser, any context]):::ui -->|/identity/*| IBFF[Identity BFF :5275]
    IBFF -->|OIDC code + PKCE| KC[Keycloak realm dialysis :8081]
    IBFF -->|"RFC 8693 token exchange<br/>(HisAccessTokenProvider, audience dialysis-his-api)"| HIS[HIS API]
    ABFF --> API[Identity API]

    API --> Prov[Provisioning slices<br/>ProvisionUser, DeactivateUser,<br/>DefineRole, Assign/RevokeRole]
    Prov --> DB[(Postgres - IdentityDbContext<br/>schema identity)]
    Prov --> OB[(Transponder outbox)]
    OB -->|RabbitMQ| Bus{{ITransponderBus}}
    Bus -->|"UserRegistered / UserDeactivated,<br/>RoleAssigned / RoleRevoked"| Downstream[Downstream consumers]
...
| `Dialysis.Identity.Bff` | **Legacy** global login BFF (`:5275`), mounted at the Gateway's `/identity/*`. Also hosts **`HisAccessTokenProvider`** (`Services/`) — RFC 8693 token exchange swapping the session's access token for one with audience `dialysis-his-api`, cached ~4 min (below the 300 s token lifespan); unique to this BFF. |
...
| `Dialysis.Module.Bff` | `AddModuleBff()` / `MapModuleBff()` — the canonical per-context BFF. `Configuration/KeycloakSecretGuard` fails startup fast outside Development when a BFF's OIDC `ClientSecret` is missing or still the `change-me` placeholder. |
...
Provisioning aggregates (schema `identity`):
...
### 6.1 Browser login (optional upstream IdP)
...
### 6.2 Authenticated API call & silent refresh
