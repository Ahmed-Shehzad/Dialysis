...
| 8 | Data management | Search | Partial | `Dialysis.HIS.DataServices` + `Dialysis.HIS.Persistence` | `SearchPatientsQuery`, EF read model | `GET …/data-management/patients/search` |
...
| 18 | Patient monitoring | Specialist medical care | Stub | — | Not modeled as its own slice | — |
...
| 25 | Generic MIS | Finance | Partial | `Dialysis.HIS.Operations` | `CreateBillingExportJobCommand`, persisted **`StatusCode`** / **`PayerCode`**, **`GetBillingExportJobByIdQuery`** | `POST …/operations/billing/export-jobs`, **`GET …/operations/billing/export-jobs/{id}`** |
...
| 28 | Generic MIS | Research & Education | Stub | — | Not implemented | — |
...
| 30 | Generic MIS | Reporting | Partial | `Dialysis.HIS.DataServices` | `ManagerDashboardQuery` with optional **`reportFocus`** echo + queued billing / open quality counts | `GET …/data-management/manager-dashboard?reportFocus=…` |
| 31 | Generic MIS | Demographics | Partial | `Dialysis.HIS.PatientFlow` | Patient + MRN on register | `POST …/patient-flow/patients` |
| 32 | Generic MIS | Reimbursement | Partial | `Dialysis.HIS.Operations` | Same billing export job surface as row 25 (**`PayerCode`**, job **`StatusCode`**, status query) | Same as finance export |
...
Additional RA surfaces not mapped 1:1 above: `GET …/capabilities/generic-mis/financial-erp-depth`, `GET/POST …/planning-and-scheduling/waitlists`, `…/medication-management/dispensing-and-barcode`, `GET …/medication-management/clinical-decision-support` + **`POST …/clinical-decision-support/evaluations`**, `GET …/data-management/full-text-and-indexing` (optional **`q`**), **`POST …/generic-mis/organizational-communications`**, **`POST …/data-management/analytics-export-jobs`**, **`POST …/patient-monitoring/advanced-alerting/{id}/clear`**, **`POST …/patient-monitoring/ehr-document-exchange/records`**, **`POST …/generic-mis/quality-workflows/{id}/status`**, **`POST …/security/mechanisms-hardening/assessments`**.
