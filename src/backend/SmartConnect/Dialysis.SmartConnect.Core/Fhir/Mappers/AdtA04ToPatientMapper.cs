...
/// <summary>
/// Maps an HL7 v2 ADT^A04 (register a patient) PID segment to a FHIR R4 <c>Patient</c>. Shape is
/// identical to ADT^A01 — the trigger event differs because some upstream systems emit A04 when a
/// patient registers as outpatient or arrives at a non-bed location without a true admission.
/// </summary>
public sealed class AdtA04ToPatientMapper : IFhirV2MessageMapper<Patient>
{
    public string TriggerEvent => "ADT^A04";
