...
/// channel handler.
/// </summary>
public sealed class PdqResponder(IPatientDemographicsResolver resolver, TimeProvider time)
{
    public async Task<string> RespondAsync(Hl7V2Message inbound, CancellationToken cancellationToken = default)
    {
        ...
        var matches = await resolver.ResolveAsync(criteria, cancellationToken).ConfigureAwait(false);
