...
/// </remarks>
public sealed class InRowAttachmentBlobStore : IAttachmentBlobStore
{
    ...
    /// callers that still drive the seam directly (e.g. legacy reattach flows).
    /// </remarks>
    public InRowAttachmentBlobStore(SmartConnectDbContext db) => _db = db;
    ...
    {
        var entity = await _db.Attachments.FirstOrDefaultAsync(a => a.Id == attachmentId, cancellationToken).ConfigureAwait(false)
    ...
    public void Write(Guid attachmentId, ReadOnlyMemory<byte> data, CancellationToken cancellationToken)
    {
        ...
        var entity = _db.Attachments.FirstOrDefault(a => a.Id == attachmentId)
            ?? throw new InvalidOperationException($"Attachment {attachmentId} does not exist; metadata row must be inserted first.");
        entity.Data = data.ToArray();
        _db.SaveChanges();
    }
    ...
    {
        var bytes = await _db.Attachments.AsNoTracking()
    ...
    {
        await foreach (var row in _db.Attachments.AsNoTracking()
