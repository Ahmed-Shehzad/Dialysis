...
public sealed class OutboxDispatcher : IAsyncDisposable
{
    ...
    public async Task StopAsync(CancellationToken cancellationToken = default)
    {
        ...
        var dispatchLoop = _dispatchLoop;
    ...
    {
        if (!TryGetDestinationKey(message, out var destinationKey) || !_activeDestinations.TryAdd(destinationKey, 0))
        {
            _ = _inflight.TryRemove(message.MessageId, out _);
            return;
        }
    ...
    private async Task PollOnceAsync(CancellationToken cancellationToken)
    {
        ...
        var pending = await session.Outbox
    ...
    {
        var destination = message.DestinationAddress?.ToString()
        ...
        try
        {
            ...
            {
                (var done, var nextAttempt) = await TryDispatchAttemptAsync(message, destination, attempt, cancellationToken).ConfigureAwait(false);
                if (done) return;
    ...
    private async Task DispatchMessageAsync(OutboxMessage message, CancellationToken cancellationToken)
    {
        ...
        var messageType = OutboxMessageTypeResolver.Resolve(message.MessageType);
    ...
    private async Task SendToDestinationAsync(
        ...
        CancellationToken cancellationToken)
    {
        var host = _hostProvider.GetHost(destinationAddress);
        var transport = await host.GetSendTransportAsync(destinationAddress, cancellationToken)
            .ConfigureAwait(false);
    ...
    private async Task PublishAsync(
        ...
        CancellationToken cancellationToken)
    {
        var publishHost = _hostProvider.GetHost(sourceAddress);
        var publishTransport = await publishHost.GetPublishTransportAsync(messageType, cancellationToken)
            .ConfigureAwait(false);
