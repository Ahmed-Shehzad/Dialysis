...
internal sealed class SagaReceiveEndpointHandler
{
        ...
        where TMessage : class, IMessage
    {
        ...
        var messageContext = TransponderMessageContextFactory.FromTransportMessage(
            ...
            sourceAddress,
            destinationAddress);
        using var scope = bus.BeginConsumeScope(messageContext);
        ...
        (var state, var isNew) = await GetOrCreateStateAsync(
            registration,
            repository,
            consumeContext,
            correlationId!.Value,
        ...
        var skipHandler = await ShouldSkipHandlerAfterStepsAsync(saga, sagaContext, cancellationToken).ConfigureAwait(false);
        if (!skipHandler)
    ...
    private async static Task<(TState? state, bool isNew)> GetOrCreateStateAsync<TState, TMessage>(SagaMessageRegistration registration,
        ...
        where TMessage : class, IMessage
    {
        var state = await repository.GetAsync(correlationId, cancellationToken).ConfigureAwait(false);
        ...
        where TMessage : class, IMessage
    {
        ...
        var steps = stepProvider.GetSteps(sagaContext) ?? [];
        var status = await sagaContext.ExecuteStepsAsync(steps, cancellationToken).ConfigureAwait(false);
        ...
        where TState : class, ISagaState
    {
        ...
        var saved = await repository.SaveAsync(state, cancellationToken).ConfigureAwait(false);
        if (saved)
            return;
