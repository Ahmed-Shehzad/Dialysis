    ...
    where TRequest : class, IMessage
{
        ...
        where TResponse : class, IMessage
    {
        ...
        try
        {
            ...
            if (completed == delayTask)
                ThrowTimeoutOrCancellation(requestId, cancellationToken);
    ...
    private void ThrowTimeoutOrCancellation(Ulid requestId, CancellationToken cancellationToken)
    {
    ...
    private Task HandleResponseAsync(IReceiveContext context)
    {
        ...
        using var scope = _bus.BeginConsumeScope(messageContext);

        if (!TryGetRequestId(message, out var requestId) || !_pendingRequests.TryGetValue(requestId, out var pending))
        {
            _logger.LogDebug(
        ...
        {
            var responseTypeName = pending.ResponseType.FullName ?? pending.ResponseType.Name;
            var messageTypeName = message.MessageType ?? "unknown";
    ...
    private static bool TryGetRequestId(ITransportMessage message, out Ulid requestId)
    {
        ...
        if (message.Headers.TryGetValue(TransponderMessageHeaders.RequestId, out var headerValue) &&
            Ulid.TryParse(headerValue?.ToString(), out var parsed))
        {
            requestId = parsed;
            return true;
        }
