...
internal static class SsePublishTargetResolver
{
        ...
        bool allowDefaultBroadcast)
    {
        ...
        var broadcast = TryGetBoolean(headers, TransponderSseHeaders.Broadcast);

        var connectionIds = GetValues(headers, TransponderSseHeaders.ConnectionId, TransponderSseHeaders.ConnectionIds);
        var excludedConnectionIds = GetValues(headers, TransponderSseHeaders.ExcludeConnectionId, TransponderSseHeaders.ExcludeConnectionIds);
        var streams = GetValues(headers, TransponderSseHeaders.Stream, TransponderSseHeaders.Streams);
        var groups = GetValues(headers, TransponderSseHeaders.Group, TransponderSseHeaders.Groups);
        var users = GetValues(headers, TransponderSseHeaders.User, TransponderSseHeaders.Users);

        if (destination is not null)
            ApplyDestination(destination, connectionIds, streams, groups, users, ref broadcast);

        var hasTargets = connectionIds.Count > 0 || streams.Count > 0 || groups.Count > 0 || users.Count > 0;
    ...
    {
        if (message.Headers.TryGetValue(TransponderSseHeaders.EventName, out var value) && value is not null)
        {
    ...
    {
        if (message.Headers.TryGetValue(TransponderSseHeaders.EventId, out var value) && value is not null)
        {
    ...
    {
        if (!headers.TryGetValue(key, out var value) || value is null) return false;
    ...
    private static List<string> GetValues(IReadOnlyDictionary<string, object?> headers, params string[] keys)
    {
        ...
        {
            if (!headers.TryGetValue(key, out var value) || value is null) continue;
            ...
            switch (value)
            {
                ...
                case IEnumerable<string> items:
                    foreach (var item in items) AddDelimited(values, item);
                    break;
                case IEnumerable<object> objects:
                    foreach (var item in objects) AddDelimited(values, item?.ToString());
                    break;
                default:
    ...
    private static void ApplyDestination(
        ...
        ref bool broadcast)
    {
        if (!TryResolveDestination(destination, out var targetKey, out var values, out var isBroadcast))
            return;
        ...
        out bool isBroadcast)
    {
        ...
        var host = destination.IsAbsoluteUri ? destination.Host : string.Empty;
        var path = destination.IsAbsoluteUri ? destination.AbsolutePath : destination.ToString();
        ...
        {
            candidate = host;
            foreach (var segment in segments) AddDelimited(values, segment);
        }
        else if (segments.Length > 0 && IsTargetKey(segments[0]))
        {
            candidate = segments[0];
            for (var i = 1; i < segments.Length; i++) AddDelimited(values, segments[i]);
        }
