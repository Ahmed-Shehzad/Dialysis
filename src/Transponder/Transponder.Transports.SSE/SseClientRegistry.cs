...
public sealed class SseClientRegistry
{
        ...
        IReadOnlyList<string> keys)
    {
        ...
        {
            if (!index.TryGetValue(key, out var ids)) continue;

            foreach (var id in ids.Keys) _ = resolved.Add(id);
        }
    }
        ...
        string connectionId)
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(key)) continue;
            if (!index.TryGetValue(key, out var set)) continue;
