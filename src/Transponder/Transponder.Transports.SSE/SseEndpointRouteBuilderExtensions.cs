...
public static class SseEndpointRouteBuilderExtensions
{
        ...
        Action<SseEndpointOptions>? configure = null)
    {
        ...
        var settings = endpoints.ServiceProvider.GetRequiredService<ISseHostSettings>();
        var path = options.Path ?? settings.Topology.StreamPath;

        return endpoints.MapGet(path, async context =>
        {
            var resolvedSettings = context.RequestServices.GetRequiredService<ISseHostSettings>();
            var registry = ResolveRegistry(context.RequestServices);
            var catchUpProvider = context.RequestServices.GetService<ISseCatchUpProvider>();
    ...
    {
        var registry = services.GetService<SseClientRegistry>();
        if (registry is not null) return registry;

        var hosts = services.GetServices<ITransportHost>();
        var sseHost = hosts.OfType<SseTransportHost>().FirstOrDefault();
