...
public sealed class GrpcTransportService : Transport.TransportBase
{
    ...
    public async override Task<SendResponse> Send(SendRequest request, ServerCallContext context)
    {
        ...
        if (!TryResolveEndpoint(destination, out var grpcHost, out var endpoint))
        {
            if (_logger.IsEnabled(LogLevel.Warning))
            {
                var knownEndpoints = string.Join(
                    " | ",
                    _hosts.Select(host =>
                    {
                        var endpoints = string.Join(", ", host.GetEndpoints().Select(entry => entry.InputAddress.ToString()));
...
#endif
        foreach ((var host, var endpoint) in endpoints)
        ...
        out GrpcReceiveEndpoint endpoint)
    {
        ...
        try
        {
            var resolved = _hostProvider.GetHost(destination);
