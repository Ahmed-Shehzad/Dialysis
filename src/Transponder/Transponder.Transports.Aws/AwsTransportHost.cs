...
public sealed class AwsTransportHost : TransportHostBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var resilientTransport = TransportResiliencePipeline.WrapSend(transport, _resiliencePipeline);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var resilientTransport = TransportResiliencePipeline.WrapPublish(transport, _resiliencePipeline);
        return Task.FromResult(resilientTransport);
    }
    ...
    internal async Task<string> ResolveQueueUrlAsync(Uri address, CancellationToken cancellationToken)
    {
        ...
        if (_queueUrls.TryGetValue(queueName, out var cachedUrl)) return cachedUrl;
    ...
    internal async Task<string> ResolveTopicArnAsync(Type messageType, CancellationToken cancellationToken)
    {
        ...
        if (_topicArns.TryGetValue(topicName, out var cachedArn)) return cachedArn;
