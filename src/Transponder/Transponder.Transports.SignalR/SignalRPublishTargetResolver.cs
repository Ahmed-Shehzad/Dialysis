...
internal static class SignalRPublishTargetResolver
{
        ...
        bool allowDefaultBroadcast)
    {
        ...
        var broadcast = TryGetBoolean(headers, TransponderSignalRHeaders.Broadcast);

        var connectionIds = GetValues(headers, TransponderSignalRHeaders.ConnectionId, TransponderSignalRHeaders.ConnectionIds);
        var excludedConnectionIds = GetValues(headers, TransponderSignalRHeaders.ExcludeConnectionId, TransponderSignalRHeaders.ExcludeConnectionIds);
        var groups = GetValues(headers, TransponderSignalRHeaders.Group, TransponderSignalRHeaders.Groups);
        var users = GetValues(headers, TransponderSignalRHeaders.User, TransponderSignalRHeaders.Users);

        if (destination is not null)
            ApplyDestination(destination, connectionIds, groups, users, ref broadcast);

        var hasTargets = connectionIds.Count > 0 || groups.Count > 0 || users.Count > 0;
    ...
    private static void AddDelimited(List<string> values, string? raw)
    {
        ...
        var filteredSegments = segments.Where(segment => !string.IsNullOrWhiteSpace(segment));
