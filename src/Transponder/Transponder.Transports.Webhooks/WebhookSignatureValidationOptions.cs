...
public sealed class WebhookSignatureValidationOptions
{
    ...
    internal string? ResolveSecret(HttpContext context)
    {
        ...
        var subscription = SubscriptionResolver?.Invoke(context);
        if (string.IsNullOrWhiteSpace(subscription))
            if (context.Request.Headers.TryGetValue(TransponderWebhookHeaders.Subscription, out var values))
                subscription = values.FirstOrDefault();

        if (!string.IsNullOrWhiteSpace(subscription) &&
            SubscriptionSecrets.TryGetValue(subscription, out var secret)) return secret;
