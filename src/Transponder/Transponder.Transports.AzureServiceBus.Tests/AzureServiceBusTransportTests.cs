...
public sealed class AzureServiceBusTransportTests
{
    ...
    public void AzureServiceBusTopology_Resolves_Queue_And_Subscription()
    {
        ...
        var queueName = topology.GetQueueName(address);
        var subscription = topology.GetSubscriptionName(address);
        var topicName = topology.GetTopicName(typeof(SampleMessage));
