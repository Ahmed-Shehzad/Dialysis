...
public sealed class ModuleBoundaryTests
{
    ...
    */
    ...
    /// <summary>
    /// SmartConnect internals must not depend on EHR, PDMS, HIS, or Identity internals (only their Contracts are allowed).
    /// </summary>
    [Fact]
    public void SmartConnect_assemblies_must_not_depend_on_other_modules_internals()
    {
        var smartConnectAssemblies = LoadByPrefix("Dialysis.SmartConnect.");
        foreach (var asm in smartConnectAssemblies)
        {
            var referenced = asm.GetReferencedAssemblies();
            var violations = referenced
                .Where(r =>
            ...
            violations.ShouldBeEmpty(
                $"SmartConnect assembly {asm.GetName().Name} must not reference non-Contracts assemblies of other modules. Violations: {string.Join(", ", violations)}");
        }
    }

    /// <summary>
    /// HIE internals must not depend on EHR, PDMS, HIS, Identity, or SmartConnect internals
    /// (only their Contracts assemblies are allowed).
    /// </summary>
    [Fact]
    public void Hie_assemblies_must_not_depend_on_other_modules_internals()
    {
        ...
        foreach (var asm in hieAssemblies)
        {
            ...
            violations.ShouldBeEmpty(
                $"HIE assembly {asm.GetName().Name} must not reference non-Contracts assemblies of other modules. Violations: {string.Join(", ", violations)}");
        }
    }
